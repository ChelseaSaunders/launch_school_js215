...
// The sample data file is a JavaScript program snippet that defines a variable
// named emailData. The value of emailData is the data you will use for this
// practice problem. To avoid editor issues (some editors have problems with
//   very long lines), we recommend that you include this file in your HTML,
//   with a script element prior to the script element that contains your code:
...
/*
create array of emails--split on "##||##"
store emailCount as emails.length
reassign to map over that, split on #/#
map over array with subarr[2]
...
function mailCount(emailData) {
  ...
  let dates = obtainDates(emails);
  dates = retriveFirstAndLastDate(dates);
  dates = processDatesForOutput(dates);
  console.log(processOutput(emailCount, dates));
}

function separateEmails(emailDataString) {
  return emailDataString.split(EMAIL_SEPARATOR)
                        .map((text) => text.split(PARTS_SEPARATOR));
}

function obtainDates(emailArray) {
  return emailArray.map((parts) => {
    let dateString = parts[2].replace(/[\\\nDate: ]/g, '');
    return new Date(dateString);
  });
}

function retriveFirstAndLastDate(dateArr) {
  dateArr.sort((a, b) => a - b).splice(1, dateArr.length -2);
  return dateArr;
}

function processDatesForOutput(dateArr) {
  return dateArr.map((date) => date.toDateString());
}

function processOutput(count, dateArr) {
  return `Count of Email: ${count}\n\n` +
  `Date Range: ${dateArr.join(' - ')}`;
}
